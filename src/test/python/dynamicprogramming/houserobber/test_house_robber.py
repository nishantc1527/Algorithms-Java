...
def test_brute_force():
    for i in range(100):
        rand_list = [random.randrange(0, 1000, 1) for _ in range(25)]  # test size has to be small since brute
...
def test_memoized():
    for i in range(100):
        rand_list = [random.randrange(0, 1000, 1) for _ in range(25)]
...
def test_bottom_up():
    for i in range(100):
        rand_list = [random.randrange(0, 1000, 1) for _ in range(25)]
        assert rob(rand_list) == bottom_up(rand_list), str(rand_list)
