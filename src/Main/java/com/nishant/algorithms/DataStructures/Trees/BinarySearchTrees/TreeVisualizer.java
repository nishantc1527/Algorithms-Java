...
//            myTree.insert((int)(Math.random()*500));
    ...
    private void setCoords() {
        ...
        this.width = ((int) ((maxLeft(tree.getRoot()) + maxRight(tree.getRoot())) *
                (this.height) * 5 * zoomFactor));
        this.setPreferredSize(new Dimension(width, height));
...
//            label.set
            ...
            if (map.containsKey(node.getLeft()))
                g.drawLine(x, y, ((int) map.get(node.getLeft()).getX()),
                    ((int) map.get(node.getLeft()).getY()));
            if (map.containsKey(node.getRight()))
                g.drawLine(x, y, ((int) map.get(node.getRight()).getX()),
                    ((int) map.get(node.getRight()).getY()));
        }
    }
