...
public class MNISTTrainer extends JPanel {
    ...
    public void run() {
        ...
        MultilayeredNeuralNetwork nn = new MultilayeredNeuralNetwork(28 * 28, 10, 0.1, 28 * 28 * 2, 28 * 28 * 2);
        ...
        for (MnistEntry entry : trainingEntries) {
            ...
            nn.train(Arrays.copyOf(inputArr, inputArr.length), Arrays.copyOf(outputArr, outputArr.length));
    ...
    private BufferedImage scale(BufferedImage img, int xscale, int yscale) {
        BufferedImage res = new BufferedImage(img.getWidth() * xscale, img.getHeight() * yscale,
